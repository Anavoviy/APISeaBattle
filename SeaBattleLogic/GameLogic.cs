...
namespace SeaBattleLogic
{
    public class GameLogic
    {
        ...
        public async Task<GameTurn> CheckTurnAsync(int userId, int idGame)
        {
            ...
            gameTurn.FieldUser = game.Creator.Id == userId ? game.FieldUser1 : game.FieldUser2;

            if (game.IdUserWinner == 0 || game.IdUserWinner == null)
                gameTurn.WinnerState = WinState.GameContinue;
            else if (game.IdUserWinner == userId)
                gameTurn.WinnerState = WinState.MyWin;
            else
                gameTurn.WinnerState = WinState.Gameover;

            if (game.IdUserNextTurn == userId)
                gameTurn.IsMyTurn = true;

            else
                gameTurn.IsMyTurn = false;
        ...
        public async Task<TurnResult> MakeTurnAsync(int userId, int idGame, int x, int y)
        {
            ...
            else if (fieldTarget[targetCell] == 2 || fieldTarget[targetCell] == 3)
            {
                turnResult = TurnResult.WasShoot;
            }
            else
            {
        ...
        public async Task<bool> CheckOpponentsAsync(int idGame)
        {
            var game = await repositoryGame.SearchEntryByConditionAsync(s => s.Id == idGame);
            if (game.Id == 0)
                throw new Exception($"not found game by id {idGame}");

            return game.Creator != null && game.Opponent != null;   
        }
    }
}